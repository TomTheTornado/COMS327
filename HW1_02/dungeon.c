...
void save_file(dungeon_t *dungeon){
    ...
    uint32_t fileSize = 1708 + dungeon->numRooms * 4 + dungeon->numUpStrs * 2 + dungeon->numDownStrs * 2;
    ...
    char *path = (char *)malloc(sizeof(char) * (strlen(getenv("HOME")) +strlen("/.rlg327/dungeon")));
    ...
    fwrite(&fileMarker, 12, 1, f);
    fwrite(&versionNum, sizeof(uint32_t), 1, f);
    fwrite(&fileSize, sizeof(uint32_t), 1, f);

    fwrite(&dungeon->pc.x, sizeof(uint8_t), 1, f);
    fwrite(&dungeon->pc.y, sizeof(uint8_t), 1, f);
    fwrite(&dungeon->hardness, sizeof(uint8_t), 1680, f);
    
    int i;
    //Writes number of rooms, locations and sizes
    fwrite(&numRooms, sizeof(uint16_t), 1, f);
    for(i = 0; i < dungeon->numRooms; i++){
        fwrite(&dungeon->rooms[i].x, sizeof(uint8_t), 1, f);
        fwrite(&dungeon->rooms[i].y, sizeof(uint8_t), 1, f);
        fwrite(&dungeon->rooms[i].length, sizeof(uint8_t), 1, f);
        fwrite(&dungeon->rooms[i].height, sizeof(uint8_t), 1, f);
    }

    //Writes upstairs then downstairs
    fwrite(&numUpStrs, sizeof(uint16_t), 1, f);
    for(i = 0; i < dungeon->numUpStrs; i++){
        fwrite(&dungeon->upstrs[i].x, sizeof(uint8_t), 1, f);
        fwrite(&dungeon->upstrs[i].y, sizeof(uint8_t), 1, f);
    }
    fwrite(&numUpStrs, sizeof(uint16_t), 1, f);
    for(i = 0; i < dungeon->numDownStrs; i++){
        fwrite(&dungeon->downstrs[i].x, sizeof(uint8_t), 1, f);
        fwrite(&dungeon->downstrs[i].y, sizeof(uint8_t), 1, f);
    }
    fclose(f);
}
...
void load_file(dungeon_t *dungeon){
    ...
    // Makes sure file types match
    if (strcmp(fileType, fileMarker)){return;}
    if (be32toh(versionNum) != 0){return;}
    ...
    for (i = 0; i < dungeon->numRooms; i++){
        ...
        uint8_t n,m;
        for (n = dungeon->rooms[i].y; n <= dungeon->rooms[i].y + dungeon->rooms[i].height; n++){
            for (m = dungeon->rooms[i].x; m <= dungeon->rooms[i].x + dungeon->rooms[i].length; m++){dungeon->map[n][m] = '.';}
        }
    }
    ...
    for (i = 0; i < max_dimY; i++){
        for (j = 0; j < max_dimX; j++){
            if ((dungeon->hardness[i][j] == 0) && (dungeon->map[i][j] == ' ')){dungeon->map[i][j] = '#';}
        }
    }
...
int main(int argc, char const *argv[]){
    ...
    dungeon.upstrs = (stairs_t *)malloc(sizeof(stairs_t)*dungeon.numUpStrs);
    dungeon.downstrs = (stairs_t *)malloc(sizeof(stairs_t)*dungeon.numDownStrs);
