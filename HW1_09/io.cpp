...
uint32_t io_look_monster(dungeon *d)
{
  pair_t dest;
  int c;
  fd_set readfs;
  ...
  mvprintw(0, 0,
           "Choose a monster location.  'g' or '.' to teleport to; 'r' for random.");
  ...
  * not otherwise used.                                                 */
  ...
  if(c != 27){
    if(charpair(dest) && d->character_map[dest[dim_y]][dest[dim_x]] 
        && charpair(dest) != d->PC){                                  
      clear();
      refresh();
      const char *name = d->character_map[dest[dim_y]][dest[dim_x]]->name;
      mvaddstr(2, 1, name);
      mvprintw(4, 1, "Speed: %d", d->character_map[dest[dim_y]][dest[dim_x]]->speed);
      mvprintw(6, 1, "%s", d->character_map[dest[dim_y]][dest[dim_x]]->description);
      
      mvaddstr(23, 1, "Press any key to continue.");
      
      getch();
    }
    else{
      io_queue_message("Monster lookout failed.  Monster not found.");
    }
  }
...
void io_inventory(dungeon *d){
  ...
  refresh();
  uint32_t i, y;
  mvprintw(2, 8, "INVENTORY");
  y = 4;
  for(i = 0; i < 10; i++){
    mvprintw(y, 15, "%d", i + 1);
    if(d->PC->inventorySlots[i] != NULL) {
      mvprintw(y, 18, d->PC->inventorySlots[i]->get_name());
    }
    y++;
  }
  refresh();
  ...
  do {
    ...
    case 'f':
     ...
     case 'm':
      ...
      // break;
    case 'w': //Wear an item
      io_queue_message("Wear an item");
      break;
    case 't': //take off an item
      io_queue_message("Take off an item");
      break;
    case 'd': //drops an item
      io_queue_message("Drop an item");
      break;
    case 'x': //Expunge item from game
      io_queue_message("Delete-Expunge item from game");
      break;
    case 'i': //List pc inventory'
      //io_inventory(d);
      io_queue_message("List inventory");
      fail_code = 1;
      break;
    case 'e': //List pc equipment
      io_queue_message("List equipment");
      break;
    case 'I': //Inspects an item
      io_queue_message("Inspect an item");
      break;
    case 'L':
      io_look_monster(d);
      fail_code = 1;
