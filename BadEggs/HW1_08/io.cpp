...
{
  uint32_t y, x;//, monType, numColor, randNum, randColor;
  ...
  for (visible_monsters = -1, y = 0; y < 21; y++) {
    for (x = 0; x < 80; x++) {
      ...
      //   if(y == (uint32_t)d->PC->position[dim_y] && x == (uint32_t)d->PC->position[dim_x]){
      //     mvaddch(y + 1, x, d->character_map[y][x]->symbol);
      //   }
      //   else if(d->character_map[y][x] != NULL){
      //     attroff(A_BOLD);
      //     monType = d->monType_map[y][x];
      //     //std::cout << d->monster_descriptions[monType].color << '\n';
      //     if(d->monster_descriptions[monType].color.size() > 0){
      //       numColor = d->monster_descriptions[monType].color.size();
      //       randNum = rand() % numColor; 
      //       randColor = d->monster_descriptions[monType].color.at(randNum);
      //     }
      //     else{
      //       randColor = 7;
      //     }
...
{
  uint32_t y, x, randColor, monType, numColor, randNum;
  character *c;
    ...
    for (x = 0; x < 80; x++) {
      if (d->character_map[y][x]) {
        if(y == (uint32_t)d->PC->position[dim_y] && x == (uint32_t)d->PC->position[dim_x]){
          ...
          //std::cout << d->monster_descriptions[monType].color << '\n';
          if(d->monster_descriptions[monType].color.size() > 0){
            numColor = d->monster_descriptions[8].color.size();
            randNum = rand() % numColor; 
            randColor = d->monster_descriptions[monType].color.at(randNum);
          }
          else{
