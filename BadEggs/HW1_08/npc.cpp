...
void gen_monsters(dungeon *d)
{
  ...
  for (i = 0; i < d->num_monsters; i++) {
    ...
    m->speed = rand_range(5, 20);                        //Need to change speed later//////////////////////////
    m->alive = 1;
    m->sequence_number = ++d->character_sequence_number;

    //monType = (rand() % (20 - lower + 1)) + lower
    while(1){
      monType = rand() % 20;
    ...
    d->monType_map[p[dim_y]][p[dim_x]] = monType;

    m->characteristics = d->monster_descriptions[monType].abilities & 0x0000000f;
    //std::cout << "Hello" << d->monster_descriptions.at(0);
